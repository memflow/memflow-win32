...
Additionally the process will be initialized with a user-provided DTB (Directory Table Base).

# Usage:
```bash
cargo run --release --example open_process_with_dtb -- -vv -c kvm -p "explorer.exe" --dtb 13f49a000
```
*/
use clap::*;
use log::{info, Level};
...
pub fn main() -> Result<()> {
    ...
    let (chain, process_name, process_dtb) = extract_args(&matches)?;
    ...
    // retrieve the extended process info for the process with a custom dtb
    let process_info = os.process_info_by_name(process_name)?;
    let process_info_ext =
        os.process_info_from_base_info_with_dtb(process_info, process_dtb.into())?;
    info!("{:?}", process_info_ext);

    // manually construct the Win32Process from process_info_ext (which contains the modified dtb)
    let mut process = Win32Process::with_kernel(os, process_info_ext);
...
fn extract_args(matches: &ArgMatches) -> Result<(ConnectorChain<'_>, &str, umem)> {
